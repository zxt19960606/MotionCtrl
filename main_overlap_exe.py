...
def corner_blending(P, overlap, cfg: AxisConfig):
    Ts = 0.001
    s1 = 0.5 * (cfg.x.Vm / cfg.x.ADCAI_T1) * (overlap * Ts)**2
    s2 = 0.5 * (cfg.y.Vm / cfg.y.ADCAI_T1) * (overlap * Ts) ** 2
    P0 = P - [s1, 0]
    P1 = P
    P2 = P + [0, s2]

    u = np.linspace(0, 1, 100)
    Pt_x = (1-u)**2 * P0[0] + 2*u*(1-u)*P1[0] + u**2 * P2[0]
    Pt_y = (1 - u) ** 2 * P0[1] + 2 * u * (1 - u) * P1[1] + u ** 2 * P2[1]
    Pt = np.column_stack((Pt_x, Pt_y))
    return Pt
# def corner_blending
...
if __name__ == "__main__":
    ...
    config = AxisConfig(x=MotionConfig(Vm=10000/60.0, Am=10000.0, Dm=10000.0, Jm=0.0, ADCAI_T1=0.2, ADCAI_T2=0.0),
                        y=MotionConfig(Vm=12000/60.0, Am=12000.0, Dm=5000.0, Jm=0.0, ADCAI_T1=0.2, ADCAI_T2=0.0))
    ...
    ##################################  拼接数据 ##############################################
    At = np.vstack((profile_1.At, profile_2.At))
    Vt = np.vstack((profile_1.Vt, profile_2.Vt))
    Pt = np.vstack((profile_1.Pt, profile_2.Pt))
    At_lap = profile_overlap_1.At
    Vt_lap = profile_overlap_1.Vt
    Pt_lap = profile_overlap_1.Pt
    #######
    ...
    ############################ 拐角过渡 ################################
    Pt_blend = corner_blending(P=np.array([100.0, 0.0]),cfg=config, overlap=overlap)
    ...
    # 程序搭接后的插补点
    fig, ax = plt.subplots()
    ax.plot([0, 100, 100], [0, 0, 80], label = 'NC contour', marker = '.', markersize = 4,
            linestyle = '-')
    ax.plot(Pt_lap[:, 0], Pt_lap[:, 1], label = 'Lap contour', marker = '.', markersize = 4, linestyle = '-')
    ax.plot(Pt_blend[:, 0], Pt_blend[:, 1], label = 'blend contour', marker = '.', markersize = 4, linestyle = '-')
    plt.grid()
    plt.axis('equal')
